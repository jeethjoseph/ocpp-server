...
**Suspend window**:
How long a mid-session charger disconnect (or reboot) is held `SUSPENDED` before force-finalize. **Per-connector-type, keyed on the `latching` trait: 48h latched / 12h unlatched** (raised 2026-09-15 from 12h / 45min; ADR 0027 amendment) — values in the git-tracked `backend/policy.py`, NOT env vars (see [[adr-0027-per-connector-type-suspend-windows]]). The same window applies on BOTH suspension paths: the disconnect timer and the post-boot timer armed by BootNotification (the old 300s post-boot timer is retired — a reboot must never shorten the promised grace window). The stale-suspended sweep and resume-staleness guard derive their cutoffs per-transaction (window + 60s buffer), preserving the [[adr-0022-resume-staleness-threshold-derived]] invariant per-row.

**Built 2026-09-15** (`disconnect_handler.last_heard_at` / `silence_seconds`, one derived clock shared by the timer `hold_until_silent`, the stale sweep and the resume guard; no column — it turned out the clock was already silence-based for readings via resume-and-re-suspend, so the change is structural): the window measures **silence, not session age** — the clock is the time since we last heard anything about *that transaction*, so a charger that checks in at hour 20 with advancing energy resets it. There is consequently **no upper bound on a session's duration**, which is correct: advancing energy is proof of life, and killing a session we can see is alive was never the intent. The window governs only the **no-information** case. The `latching` split **survives** — it remains exactly the right discriminator when we know nothing, since a latched cable means the car is probably still attached and an unlatched socket means anyone could have pulled it — and the magnitudes are now 48h latched / 12h unlatched — every window-killed latched outage in a 90-day prod+staging sweep came back within 48h, 93% of unlatched within 12h; no hard session-age ceiling (advancing energy is proof of life, money is bounded by the Budget cap). `suspended_at` is not the clock; it is the sweep's pre-filter and the Boot path's "heard from" stamp (flap-capped). A Heartbeat, StatusNotification or bare WebSocket reconnect does **not** extend the window — alive is not the same as session-alive, and continuity firmware replays immediately on reconnect.
...
It covers only *surviving* an outage mid-session, never *beginning* one during it, because every session needs a funding decision only the CSMS can make. In this fleet that is true **by construction, not by configuration** (decided 2026-09-15): no charger has a local start path (no RFID tap, no button — the CSMS implements no `Authorize` handler), every session begins with a `RemoteStartTransaction`, and an offline charger cannot receive one. OCPP 1.6's offline-start keys (`AllowOfflineTxForUnknownId`, `LocalAuthorizeOffline`) are therefore unreachable and are NOT asserted; the assertion is built only if firmware ever gains a local start path. **No per-charger configuration from the CSMS (decided 2026-09-15).** Charger behaviour is fleet-wide and lives in firmware; the CSMS does not send `ChangeConfiguration` / `GetConfiguration` and has no per-charger configuration surface. Standard OCPP keys may still name firmware behaviour so it is legible to OCPP tooling, but their values are firmware defaults, not something the CSMS sets.
...
Reported energy is recorded, never promoted: it does not overwrite the billed figure and does not move money. **Built 2026-09-15 (migration 64):** `Transaction.reported_end_meter_kwh` / `reported_energy_kwh`, stamped equal to the billed pair on a normal stop and written alone on a late one; `models.TERMINAL_TRANSACTION_STATES` is the single definition of "money is frozen" that the finalizer and both OCPP handlers share. Replayed `MeterValues` for a terminal session are stored and acknowledged but never resume, budget-check or bill, and only ever raise the reported figure. An issued **GST Invoice** cannot be corrected — there is no credit-note table (dropped in migration 27) and stamping a correction breaks the register invariant — so a register that silently disagreed with its own transactions would be strictly worse than a known, measured gap. Reported energy exists to quantify what the write-off is costing and to alert on charger-side meter failure.
...
The charger **owns the meter**; the CSMS observes and does not correct. A violation is logged as an alertable event and **billed as reported** — the meter is the instrument of record. *(Built 2026-09-15: `services/meter_monotonicity.observe_reading`, Redis high-water mark per transaction, audit `transaction.meter_regression`, NR event `OCPPMeterMonotonicityViolation`.)* Observation is not overruling: with `PostBootState` retired there is no recovery fallback and no credit note, so this is the only signal that charger-side meter persistence has failed in the field.
...
**Key provisioning** vs **Key rotation**:
Two distinct operations on a **Charger Auth Key**, deliberately not one. **Provisioning** mints the first key for a charger that has none — safe, reversible in effect, and never destroys anything. **Rotation** replaces an existing key, destroying the only credential the unit in the field still holds; it has no grace overlap, so the charger fails authentication from that instant until the new key is loaded onto it by charger-side tooling. Because the fleet sits behind carrier NAT with no inbound path, an unintended rotation is recovered by physically visiting the unit.
_Avoid_: "overwrite" and "regenerate" — both flatten the two into one act and hide that only one of them is destructive. _Avoid_: treating a rotation of an unprovisioned charger as a provisioning; there is nothing to rotate.
...
**Transfer fee** (Route):
Razorpay's charge for moving a **Settlement Entry**'s payout to the franchisee's linked account — `fees + tax` on the Transfer entity, stored as `CommissionLedgerEntry.transfer_fee`. **VoltLync's own cost, debited from the platform**, never deducted from the franchisee's credit, and legitimately ₹0 on small amounts. Captured at `transfer.processed`, where the entity already carries it. It is a *different fee from a different party* than the **Gateway fee**: the Gateway fee is customer-borne and cancels out of the franchisee's payout; the Transfer fee is a platform expense and must **never** be netted against `franchisee_payout`.
_Avoid_: "settlement fee" (the `settlement.processed` entity's `fees`/`tax` are settlement-level aggregates, not per transfer); folding this into the Gateway fee.
...
**`TRANSFER_PROCESSED` is not "paid".** Razorpay's `transfer.processed` means the transfer was accepted; the money reaches the linked account's bank only when that account's own settlement runs (the parent's T+n schedule, after any hold lifts). **`SETTLED` is reached by lookup, not by message**: no Razorpay event lists which transfers a settlement paid — `settlement.processed` carries only the settlement's own `id/amount/status/fees/tax/utr`, and the link runs transfer → settlement. The CSMS therefore asks the transfers (`GET /v1/transfers?recipient_settlement_id=` on the webhook, and a six-hourly poll of each `TRANSFER_PROCESSED` row older than the sweep floor) and advances a row only when the transfer says `settled` **and** the settlement it joined says `processed`. Between June and September 2026 the handler read a `transfers` array that does not exist, and no row ever reached `SETTLED` (377 rows, ₹18,130) — and nothing alerted, because until 2026-09-14 the stuck-payout detector's definition of "stuck" had no `TRANSFER_PROCESSED` clause. It now flags a row processed more than `STUCK_PROCESSED_DAYS` ago; the four tuning values live together in `backend/policy.py` because the alarm must sit strictly past the sweep floor.
_Avoid_: conflating "earned" (the entry exists) with "settled" (the status reached its terminal state); reading `TRANSFER_PROCESSED` as money-in-bank; expecting a `transfers` list on any settlement-shaped entity.
...
- "overwrite" was used for replacing a **Charger Auth Key** — resolved: the operation is **Key rotation**, and it is distinct from **Key provisioning**. Only rotation is destructive.
