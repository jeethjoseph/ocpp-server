...
**2026-07-14 — CLOSED / not shipped.** The diagnostic was built on a feature branch but **never merged**, and was subsequently **retired** — see ADR 0002 (2026-06-18 amendment): the thin-settlement-float hypothesis was disproven (downgrades occur at high balance while instant succeeds at low), so the funding-pool snapshot (`fetch_balance`, `_fetch_funding_pools`, `balance_before`/`refund_credits_before`) and the creation-time `QRRefundSpeed` event were dropped in favour of `QRRefundFinalSpeed` recorded from the terminal `refund.processed` webhook. The "Implemented … not yet committed" note above is superseded: none of this code is in the current tree, and that is intentional. This issue is CLOSED.
