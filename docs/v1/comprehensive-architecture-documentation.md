...
- For `charging` / `paused` / `stopping` rows, the endpoint computes live KPIs by reading the existing **`qr_session:{txn_id}` Redis cache** (`tariff_rate` / `gst_percent` / `platform_fee` (synthetic) / `budget_limit_paise` / `start_meter_kwh`) and the latest `MeterValue` row for `reading_kwh` / `power_kw`. Computed fields: `energy_kwh`, `spent_so_far` = `energy × rate × (1 + gst/100) + synthetic_platform_fee`, `refund_if_stopped_now` = `max(0, amount_paid - spent_so_far)`, `power_kw`. `waiting` entries instead carry `stale_threshold_seconds` (remaining seconds until the stale-watchdog auto-refund fires).
