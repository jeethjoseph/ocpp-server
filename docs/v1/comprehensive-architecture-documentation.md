...
> **⚠️ SUPERSEDED 2026-07-13 by [ADR 0026](../adr/0026-tariff-excludes-gateway-actual-fee.md).** Every "synthetic 2% fee", "all-in tariff", `tariff_per_kwh_all_in`, `RAZORPAY_PLATFORM_FEE_PERCENT`, `back_derive_rate_per_kwh`, and `tariff_drift_check` reference in the tariff/billing/settlement subsections below is obsolete. Now: `Tariff.tariff_per_kwh_all_in` → `Tariff.rate_gst_included` (GST-inclusive, **gateway-exclusive**); `rate_per_kwh` = `rate_gst_included / (1 + gst%/100)` via `back_calc_base_rate` (no gateway factor). The synthetic fee, its env var, its startup validation, and the drift checker are all removed. The gateway is the **actual** webhook Razorpay fee (`QRPayment.platform_fee`/`razorpay_commission`/`razorpay_gst`), billed as a separate invoice line, reserved from the budget, deducted in the refund, and used as the settlement `pg_fee` (it cancels out of the franchisee payout). QR energy = `kWh × base_rate` → effective ₹/kWh == advertised tariff at any consumption. `GSTInvoice.tariff_per_kwh_all_in` → `rate_gst_included`.
...
- **Invoice tariff snapshot (post-2026-05-19)**: `GSTInvoice.tariff_per_kwh_all_in` (nullable, added by migration 38) captures the operator's `Tariff.tariff_per_kwh_all_in` at the moment of issuance. Since the itemised-layout redesign (ADR 0024) it is shown as a discreet "Tariff quoted (all-inclusive)" note above the line-item table (no longer a table column), so the customer still sees the all-in rate they were quoted alongside the GST-exclusive per-kWh Rate in the table. Legacy invoices (pre-cutover) have NULL and omit the note. See ADR 0003 "Invoice display."
- **Itemised invoice line-item table (2026-07-09, ADR 0024)**: the PDF renders a full-width `HSN · Item · Rate · Qty · Taxable Value · SGST · CGST · Line total` table (single **IGST** column for inter-state), rows Energy + Gateway charges (gateway omitted for wallet). Taxable Value is the pre-tax per-line amount (`Taxable + tax = Line total`); the Gateway line shows Rate as a flat `%` of the amount paid (Qty = amount paid). The Energy **Rate** is the GST-exclusive figure, derived on-page as `energy_taxable_value ÷ energy_consumed_kwh` (== `rate_per_kwh`), so it works for legacy invoices with no stored rate. Per-line SGST/CGST is a **display allocation** of the stored total-level tax (ADR 0017 untouched): Energy takes `round(taxable × rate%)`, Gateway absorbs the residual, so each head sums exactly to the stored `cgst/sgst/igst_amount`. Line total is tax-inclusive; the footer keeps Sub Total → Round Off → TOTAL, with TOTAL still reconciling to `amount_paid − refund` for QR. No schema change. The shared `build_invoice_line_items(invoice)` helper is reused by the `/my-charges` receipt card. See `docs/adr/0024-itemised-gst-invoice-layout.md`.
...
- Backfill (migration 59) applies an **explicit 22-row map keyed on `charge_point_string_id`**, never `Charger.id` — row ids differ per register and can be reused after a delete. `upgrade()` never reads `Charger.name`; the map is literal data, so what a reviewer reads is exactly what executes. Four guards raise rather than skip.
...
A **GST Invoice** now snapshots three charger facts. `charger_id_str` keeps holding exactly what was *printed* — a UUID before the cutover, an Asset Code after, the two eras separable by `^VOWS?\d{4,}$` — and keeps its name despite the Asset Code avoid-list, because it is a column header in the GST filings CSV that feeds an accountant's spreadsheet. `charger_station_id` and `charger_ocpp_id` join it, both internal and never exported; the latter preserves the audit link to the physical unit across the cutover. Migration 61 rewrites `charger_id_str` **only where `pdf_url IS NULL`** — an issued invoice is a frozen tax document — while backfilling the two internal columns everywhere, precisely because they are never rendered.

**Known residue**: the QR landing page is still `/charge/{charge_point_string_id}`, and that URL is encoded in the QR stickers already on the fleet, so the UUID stays publicly reachable even though nothing renders it. This is **not** merely a latent ADR 0020 concern: `POST /api/diagnostics/bundles` is live (ADR 0029) and `charger_auth_service.authenticate_charger` requires the Basic Auth username to equal `charge_point_string_id`, so those UUIDs are enumerable usernames on a production authentication surface. The only thing behind them is the 20-byte Charger Auth Key, stored as a SHA-256 hash, with a null hash rejected outright — so this is exposure, not an open door. Closing it needs a public handle that is not the auth username plus a fleet re-sticker; tracked at `.scratch/charger-asset-code/issues/10-qr-url-still-embeds-the-uuid.md`.
...
- **No `GSTInvoice` is issued for any fully-refunded session (ADR 0023, 2026-07-09).** After the `energy <= 0` short-circuit, `generate_invoice` gates on the **net amount retained**: it suppresses issuance when `amount_paid - refund_amount ≤ 0` (QR) or `total_billed ≤ 0` (wallet), returns `None`, consumes no invoice number, and increments `Custom/Invoice/NonBillableSkipped`. This fixed the case where a fault-refunded session (ADR 0013 band: `FAILED`, `0 < energy < 0.5 kWh`, `energy_charge` = 0) still produced a phantom-gateway-only invoice because the old guard checked *metered* energy (> 0) instead of the *billed* amount — observed as prod invoice `VL/F3/QR/202627/00061` (paid ₹50, refunded ₹50, TOTAL ₹1.00, violating `total_amount + refund_amount == amount_paid`). Net-retained is stronger than `energy_charge > 0`: it also suppresses goodwill/manual full refunds of otherwise-billed sessions. See `docs/adr/0023-invoice-only-when-net-retained-positive.md`.
