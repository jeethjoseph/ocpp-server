...
**Known residue**: the QR landing page is still `/charge/{charge_point_string_id}`, and that URL is encoded in the QR stickers already on the fleet, so the UUID remains publicly reachable even though nothing renders it. Closing it needs a public handle that is not the auth username plus a fleet re-sticker — effectively a prerequisite of ADR 0020, tracked at `.scratch/charger-asset-code/issues/10-qr-url-still-embeds-the-uuid.md`.
