...
>
> **Update (2026-08-18):** the *credential* half of this ADR is being shipped ahead of the handshake half by [ADR 0029](0029-diagnostic-bundle-authenticated-https-upload.md), which provisions `Charger.auth_key_hash` fleet-wide and uses HTTP Basic Auth with the Charger Auth Key to authenticate **Diagnostic Bundle** uploads over HTTPS. The OCPP WebSocket handshake described here stays unauthenticated for now, so the identity-spoofing and DoS exposures below remain open. What is left of this ADR after 0029 is the handshake check itself, against a column that already exists and is already populated — plus the `REQUIRE_CHARGER_AUTH` burn-down. Note that 0029 enforces with **no legacy grace** on its own endpoint; the per-charger legacy mode described below applies only to the WSS path.
