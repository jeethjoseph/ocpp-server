...
> **Amended 2026-09-15 (ADR 0031, offline-continuity issue 03): magnitudes raised to 48h latched / 12h unlatched.** The per-type split, the git-tracked policy home and both-paths rule below all stand. What changed is what the window *means*: under ADR 0031 it measures silence, not session age, and past it the session is written off — so it is "how long we hold a customer's money before writing off", and the cost of a longer window is refund latency on an abandoned session, not cable security (a pulled plug under continuity firmware is a flat meter that bills nothing). Evidence: a 90-day sweep of every session the 12h / 45min windows killed, prod + staging, measuring time until the charger next spoke — latched 15 sessions, longest 41h, all back within 48h; unlatched 42 sessions, 93% back within 12h, all within 48h. Unlatched stays materially shorter because a bare socket gives no evidence the vehicle is still attached, so the abandoned-session refund should not wait two days. **No hard session-age ceiling** was added: advancing energy is proof of life and money is bounded by the Budget cap; the flap guard caps a rebooting-without-replay charger at roughly four windows, and under continuity that pattern is a meter-persistence failure for the monotonicity alerting to surface. The "QR money can be held up to 12h" trade-off below is now 48h.
